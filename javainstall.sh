...
create_javaws_shortcut() {
    shortcut_file="$applications_dir/javaws.desktop"
    cat <<_EOF_ >$shortcut_file
[Desktop Entry]
Name=Oracle Java Web Start
...
if [[ -d $applications_dir ]] && [[ -f $extracted_dirname/bin/javaws ]]; then
    if (confirm "Do you want to create a desktop shortcut to javaws?"); then
        create_javaws_shortcut
    fi
fi
...
create_java_shortcut() {
    shortcut_file="$applications_dir/java.desktop"
    cat <<_EOF_ >$shortcut_file
[Desktop Entry]
Name=Oracle-Java
GenericName=Sun-Java
Comment= jar çalıştırma dosyası
...
if [[ -d $applications_dir ]] && [[ -f $extracted_dirname/bin/java ]]; then
    if (confirm "Do you want to create a desktop shortcut to java?"); then
        create_java_shortcut
    fi
fi
...
create_javacontrol_shortcut() {
    shortcut_file="$applications_dir/javacontrol.desktop"
    cat <<_EOF_ >$shortcut_file
[Desktop Entry]
Encoding=UTF-8
...
if [[ -d $applications_dir ]] && [[ -f $extracted_dirname/bin/javacontrol ]]; then
    if (confirm "Do you want to create a desktop shortcut to javacontrol?"); then
        create_java_shortcut
    fi
fi
